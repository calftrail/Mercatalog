...
- (id)initWithURL:(NSURL*)projectURL options:(TLProjectBundleFlags)flags error:(NSError**)err {
	...
	if (self) {
		...
		BOOL didCreate = NO;
		(void)[self ensureExistenceOfDirectory:fullPath
		...
		if (!internalError) {
			...
			if (didCreate) {
				(void)[self createWithOptions:flags error:&internalError];
			}
			else {
				(void)[self loadWithOptions:flags error:&internalError];
			}
		}
							 ...
							 error:(NSError**)err
{
	...
	if (pathExists && !isDirectory) {
		...
		createdDir = [[NSFileManager defaultManager] createDirectoryAtPath:fullPath withIntermediateDirectories:YES attributes:packageAttributes error:&internalError];
	}
	else {
